...
export class Tool10Component implements OnInit {
  ...
  addToCart() {
    if (this.option1 == "Electrically Safe Rubber Handle") {
      let cart = {item: "Channel Lock Pliers", price: this.price, quantity: this.quantity, use: this.option1,  final: this.final};
      this.shoppingcartservice.addItem(cart);
    }
    else if (this.option2 == "Fiber Glass Wrapping"){
      let cart = {item: "Channel Lock Pliers", price: this.price, quantity: this.quantity, use: this.option2,  final: this.final};
      this.shoppingcartservice.addItem(cart);
    }
